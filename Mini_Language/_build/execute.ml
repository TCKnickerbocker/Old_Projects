...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | For (str1, e1, e2, statement) -> 
    (* ( match (eval e1 (makeEnv stt []), eval e2 (makeEnv stt []) ) with
    | Int x, Int y -> ( match eval (Lte(Val(Int x), Val (Int y) )) (makeEnv stt []) with 
      | Bool true -> exec (For (str1, e1, e2, statement)) (exec statement stt)
      | Bool false -> stt
      | _ -> failwith "Incompatible types in For" )
    | _ -> failwith "Incompatible types in For" )
     *)
  ...
  | RepeatUntil (statement, e) -> (match eval e (makeEnv stt []) with 
    | Bool true -> stt
    | Bool false -> exec (RepeatUntil(statement, e)) (exec statement stt)
    | _ -> failwith "Incompatible types in RepeatUntil" 
    ) 
  | DoWhile (statement, e) -> (match eval e (makeEnv stt []) with 
